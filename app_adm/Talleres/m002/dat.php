...
from v_aludgr_taller where aludgr_apepat+' '+aludgr_apemat+' '+aludgr_nombres like '$q%'");
    ...
    function ins_alu($idalumno,$fecpag,$idprograma,$partes,$canti,$resta,$obs,$cxp) {
        if (empty($idalumno) or empty($fecpag) or empty($idprograma) or ($canti==""))
            ...
            $numrecp=$this->bd->fetch_cell("SELECT ISNULL(max(right(id_pago,3))+1,1) from TB_TALLER_PAGOS");
            $numrecp=substr($ano,-2).str_pad($numrecp,3,'0',STR_PAD_LEFT);
            ...
            if(count($cxp)==0){
                ...
                $this->bd->exe_sql($sql);
                $r = array( "exito"=>"1", "mensaje"=>"Se matriculo al alumno con exito","numrec"=>$numrec,"paquete"=>"0");
            }
            else{
                foreach ($cxp as $key => $value) {
                    $numrec=$this->bd->fetch_cell("SELECT ISNULL(max(right(id_pago,3))+1,1) from TB_TALLER_PAGOS");
                    $numrec=substr($ano,-2).str_pad($numrec,3,'0',STR_PAD_LEFT);
                    $sql="insert into TB_TALLER_PAGOS(ID_PAGO,ID_ALUMNO,D_FECPAG,ID_PROGRAMACION,C_PARTES,N_CANTIDAD,N_RESTA,C_DEBE,ID_USER,C_OBS,C_PAQUETE)
                        VALUES('$numrec','$idalumno','$fecpag','$key','$partes','$value','$resta','$debe','$_SESSION[N_USUCOD]','$obs','$numrecp');";
                    $this->bd->exe_sql($sql);
                    //$this->bd->exe_sql("INSERT INTO tb_taller_pago_paquetes(ID_PAGO,ID_PROGRAMACION)VALUES('$numrec','$key')");
                    $numrec_paq.=$numrec.",";
                }
                $r = array( "exito"=>"1", "mensaje"=>"Se matriculo al alumno con exito","numrec"=>$numrec_paq,"paquete"=>"1");
            }
        }
        echo json_encode($r);
    }
